...
public class CakeService(ApplicationDbContext context):ICakeService {
 ...
 public async Task<bool> AddCake(Cake cake){ if(await context.Cakes.AnyAsync(x=>x.Name==cake.Name)) return false; context.Cakes.Add(cake); await context.SaveChangesAsync(); return true; }
